<template>
  <div>
    <div class="action">
      <el-card>
        <div class="update">
          <el-divider><strong><i class="el-icon-s-promotion"></i>更新动作</strong></el-divider>
          <el-row :gutter="20">
            <template  v-for="(val, key) in actionName">
              <el-col :span="1.5" :key="key">
                <el-button  type="primary" size="small" :icon="val.icon" @click.native="assetsActions(val.action, val.name)">{{ val.name }}</el-button>
              </el-col>
            </template>
          </el-row>
        </div>
          ...
          <el-table :data="dataList" border strip v-loading="loading" @selection-change="handleSelectionChange" :header-cell-style="{background: '#FAFAFA'}" element-loading-text="拼命加载中">
            ...
            <el-table-column prop="runStatus" label="更新状态" width="110">
              <template slot-scope="scope">
                <el-tag v-if="scope.row.runStatus === 100" type="info">未更新</el-tag>
                <el-tag v-if="scope.row.runStatus === 200" type="success">更新成功</el-tag>
                <el-tooltip class="item" effect="dark" content="更新中" placement="right-start">
                  <i v-if="scope.row.runStatus === 300" class="el-icon-loading" style="font-size: 17px;color: red"></i>
                </el-tooltip>
                <el-tag v-if="scope.row.runStatus === 400" type="danger">更新失败</el-tag>
              </template>
            </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      // 更新的动作可以放到数据库里
      actionName: [
        { action: 'GameUpdate', icon: 'el-icon-s-tools', name: '停机维护' },
        { action: 'DockerUpdate', icon: 'el-icon-takeaway-box', name: 'docker更新' },
      ],
      items: [{
          value: 'test',
          label: 'test'
  ...
  methods: {
    ...
    // 更新
    assetsActions (action, name) {
      if (this.multipleSelection.length === 0) {
        return this.$message.error('请先勾选需要更新的服务器.')
      }
      elConfirm(`确定操作【${name}】吗？`).then(() =>{
        for (let i=0; i<this.multipleSelection.length; i++) {
          let routeData = this.$router.resolve(
            { path: `/assets/action/${this.multipleSelection[i].project}/${this.multipleSelection[i].ip}/${action}` }
            );
    ...
    // 输入参数更新
    assetsArgsActions (action, name) {
      if (!this.deployItem.projectName) {
        return this.$message.error('请输入项目名')
      }
      if (!this.deployItem.dbPort) {
        return this.$message.error('请输入数据库端口')
      }
      if (this.multipleSelection.length === 0) {
        return this.$message.error('请先勾选需要更新的服务器.')
      }
      let info = `${this.deployItem.projectName}|${this.deployItem.dbPort}`
      elConfirm(`确定操作【${name}】吗？`).then(() =>{
        for (let i=0; i<this.multipleSelection.length; i++) {
          let routeData = this.$router.resolve(
            { path: `/assets/action/${this.multipleSelection[i].project}/${this.multipleSelection[i].ip}/${action}/${name}/${info}` }
            );
          window.open(routeData.href, '_blank');
        }
      }).catch( () => {
